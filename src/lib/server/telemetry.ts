...
import { readProjects } from '@/lib/server/projectMetrics';
import { readRunStats } from '@/lib/server/jobRunMetrics';
import { readFleetAgents } from '@/lib/server/fleetMetrics';
...
class EstateSimulator {
  ...
  private seed() {
    ...
    // Finance is not tracked anywhere real yet — no accounts, no balance
    // history. It stays empty and zero rather than showing fabricated money.
    this.vaultQueries = [];
    ...
    const runStats = readRunStats();
    this.automationRuns24h = runStats.runs24h;
    this.automationFailures24h = runStats.failures24h;
  ...
  advance() {
    ...
    /* --- Agents (real: personas + job store, see fleetMetrics.ts) --------- */
    this.agents = readFleetAgents();
    ...
    /* --- Projects (real: vault notes, see projectMetrics.ts) -------------- */
    this.projects = readProjects();

    /* --- Automation counters (real: job store, see jobRunMetrics.ts) ------ */
    const runStats = readRunStats();
    this.automationRuns24h = runStats.runs24h;
    this.automationFailures24h = runStats.failures24h;
  ...
  private generateInsights(initial: boolean) {
    ...
    candidates.push({
      ...
      source: 'SAM · service mesh',
      evidence: `${degraded[0]?.latencyMs ?? 0}ms p50`,
      suggestedAction: 'Restart the container before the queue backs up further.',
    });

    // vault-backlog and runway are gated out: no embedding pipeline exists, so
    // pendingIndex is every note (would fire permanently); finance is not
    // tracked anywhere real, so runway has no inputs.
  ...
  // commandAgent was removed: it fabricated agent state (throttled/offline/
  // resume with a fake task pool) that had no real-world source or consumer.
